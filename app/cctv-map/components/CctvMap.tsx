...
export default function CctvMap({ isAdminMode = true }: CctvMapProps) {
  ...
  const regeneratePdf = async (): Promise<string | null> => {
    try {
      // Check if we have a valid cached PDF (all images unchanged)
      console.log('[CctvMap] ตรวจสอบ cache PDF...');
      const cachedPdfUrl = checkPdfCacheValid(cameraItems);
      
      if (cachedPdfUrl) {
        console.log('[CctvMap] ใช้ PDF จาก cache');
        setCachedPdfUrl(cachedPdfUrl);
        setIsPdfOutdated(false);
        setNewPdfUrl(cachedPdfUrl);
        setPdfReady(true);
        return cachedPdfUrl;
      }
            ...
            {pdfReady && newPdfUrl ? (
              <>
                ...
                <div className="flex flex-col gap-3 w-full">
                  ...
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(newPdfUrl);
